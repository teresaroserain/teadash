...
import sourcefiles.src.utils.transactions as transactions
import sourcefiles.src.visualization.simplegraph as simplegraph
import sourcefiles.src.visualization.networkgraph as networkgraph
import sourcefiles.src.utils.sqlqueries as sqlqueries
import sourcefiles.src.algorithms.DecisionTree as DecisionTree
import sourcefiles.src.utils.functions as functions
import os.path as Path
import dash_auth
from sourcefiles import app
import urllib.parse
...
####for i, course in df_courses.iterrows():
    ####if not np.isnan(course["CourseId"]):
        ...
        #""" if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/interaction.csv')) == True:
            #print(os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/participation.csv')))
            #df_interaction = pd.read_csv(os.path.join(os.path.dirname(__file__) +'/Datasets/interaction.csv'), delimiter=";")
            #df_discussionparticipation = pd.read_csv(os.path.join(os.path.dirname(__file__) +'/Datasets/discussionparticipation.csv'), delimiter=";")
        #else: """
        ...
        # ======= DataFrame Pageview =======
        #""" if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/df_pageviews.csv')) == True:
        #    df_pageviews = pd.read_csv(os.path.join(os.path.dirname(__file__) +'/Datasets/df_pageviews.csv'), delimiter=";")
        #else:  """
        ####df_pageviews = transactions.get_pageviews(course)
        # ======= DataFrame Analytics Assignments =======
        #""" if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/analyticsassignments.csv')) == True:
        #    #print(os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/participation.csv')))
        #    df_analyticsassignments = pd.read_csv(os.path.join(os.path.dirname(__file__) +'/Datasets/analyticsassignments.csv'), delimiter=";")
        #else:  """
        ####df_analyticsassignments = transactions.get_analyticsassignments(course)
        
        # ======= DataFrame Participations =======
        #""" if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/participation.csv')) == True:
        #    #print(os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/participation.csv')))
        #    df_participation = pd.read_csv(os.path.join(os.path.dirname(__file__) +'/Datasets/participation.csv'), delimiter=";")
        #else: """ 
                    ...
                    html.Div(id='div1',
                        children = [
                            ...
                            html.Div( 
                                    className='graph',
                                    children=[
                                        html.Div(id='title_userviews'),
                                        html.Div(dcc.Graph(id='userviews')) 
                                    ],   
                            ),
                            html.Div( 
                                    className='graph',
                                    children=[
                                        html.Div(id='title_userparticipation'),
                                        html.Div(dcc.Graph(id='userparticipation',))      
                                    ],   
                            ),
...
# ======= defines callbacks =======
@app.callback(
              Output('title_userviews','children'),
              Output('title_userparticipation','children'),
              Input('url', 'search'),
              )
def display_page(pathname):
    ...
    course = {'CourseId': int(parsed_dict['courseid'][0]), 'CourseCode':'', 'CourseName':''}
    print("course ", course)
    
    return 'User View', 'User Participation'
...
#for HV
@app.callback(
        Output('userviews', 'figure'),
        Input("retrieve_val", "n_clicks"), 
        )
def showParallelPlot_Assignments(n_clicks):
    if n_clicks is not None: 
        df_useractivities = transactions.get_useractivities(course)
    else:
        if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/useractivity_'+ str(course["CourseId"]) +'.csv')) == True:
            df_useractivities = pd.read_csv(os.path.join(os.path.dirname(__file__) +'/Datasets/useractivity_'+ str(course["CourseId"]) +'.csv'), delimiter=";")
        else: 
            df_useractivities = transactions.get_useractivities(course)
    if len(df_useractivities.index) != 0:
        df = df_useractivities[['StudentName', 'Type', 'Title']]
        return simplegraph.showParallelPlot_Assignments(df) 
    
@app.callback(
        Output('userparticipation', 'figure'),
        Input("retrieve_val", "n_clicks"), 
        )
def showParallelPlot_Quiz(n_clicks):
    if n_clicks is not None: 
        df_interaction, df_discussionparticipation = transactions.get_interactions_discussionview(course)
    else: 
        if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/discussionparticipation_'+ str(course["CourseId"]) +'.csv')) == True:
            #print(os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/participation.csv')))
    ...
    """ if n_clicks is not None: 
        ...
        if os.path.exists(os.path.join(os.path.dirname(__file__) +'/Datasets/participation_'+ str(course["CourseId"]) +'.csv')) == True:
            ...
            df_participation = transactions.get_participations(course) """
    if len(df_discussionparticipation.index) != 0:
        df = df_discussionparticipation[['StudentName', 'Title']]
        return simplegraph.showParallelPlot_Assignments(df)
...
def showBarGraph(n_clicks):
    ...
    if len(df_participation.index) != 0:
        ...
        df = sqlqueries.sql_studentparticipationHV(df) 
...
def showLineGraph_courseaccess(data, updatedtime, n_clicks):
...
def showNetworkGraph(n_clicks):
    ...
    if n_clicks is not None: 
        ...
        return networkgraph.networkGraph(df_interaction, False), noteNetwork, noti, df_interaction.to_dict('records'), df_discussionparticipation.to_dict('records')
    raise PreventUpdate
...
def showParallelIndividuals(selectedData, n_clicks, stored_analyticsassignments, stored_interaction):
    data = []
    columns = []
